...
export function prepareAuthPolicies({
	...
	const green: Policy<GreenMeta, GreenAuth> = async(meta, request) => {
		return {bakeTables}
	}

	// policy for authorized anonymous user
	const anon: Policy<AnonMeta, AnonAuth> = async({accessToken}, request) => {
	...
	const user: Policy<UserMeta, UserAuth> = async(meta, request) => {
		const auth = await anon(meta, request)
		if (auth.access.user)
			return auth
		else
	...
	const userWhoManagesPermissions: Policy<UserMeta, UserAuth> = async(meta, request) => {
		const auth = await user(meta, request)
		auth.checker.requirePrivilege("customize permissions")
		return auth
	}

	// user who is allowed to view statistics
	const statsViewer: Policy<UserMeta, UserAuth> = async(meta, request) => {
		const auth = await user(meta, request)
	...
	const platformUser: Policy<PlatformUserMeta, PlatformUserAuth> = async(meta, request) => {
		const auth = await user(meta, request)
	...
	const appOwner: Policy<
		...
		const auth = await platformUser(meta, request)
